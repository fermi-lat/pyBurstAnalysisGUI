...
thisCommand.addParameter("filteredeventfile","Input event list (FT1 file)",mandatory=commandDefiner.MANDATORY,partype=commandDefiner.DATASETFILE,extension="fit")
thisCommand.addParameter("rspfile","LAT response (RSP file)",mandatory=commandDefiner.MANDATORY,partype=commandDefiner.DATASETFILE,extension="rsp")
thisCommand.addParameter("ft2file","Spacecraft file (FT2)",mandatory=commandDefiner.MANDATORY,partype=commandDefiner.DATASETFILE,extension="fits")
thisCommand.addParameter("expomap","pre-computed exposure map",mandatory=commandDefiner.OPTIONAL,partype=commandDefiner.DATASETFILE,extension="fits")
thisCommand.addParameter("ltcube","pre-computed livetime cube",mandatory=commandDefiner.OPTIONAL,partype=commandDefiner.DATASETFILE,extension="fits")
thisCommand.addParameter("xmlmodel","XML model",mandatory=commandDefiner.MANDATORY,partype=commandDefiner.DATASETFILE,extension="fits")
...
thisCommand.addParameter("tsmin","Minimum TS to consider a source detected",mandatory=commandDefiner.OPTIONAL,defaultValue=20)
thisCommand.addParameter("optimizeposition","Optimize position?",mandatory=commandDefiner.OPTIONAL,defaultValue="no",possiblevalues=['no','yes'])
thisCommand.addParameter("showmodelimage","Show an image representing the best fit likelihood model?",mandatory=commandDefiner.OPTIONAL,defaultValue="no",possiblevalues=['no','yes'])
thisCommand.addParameter("spectralfiles","Produce spectral files for XSPEC?",mandatory=commandDefiner.OPTIONAL,defaultValue="no",possiblevalues=['no','yes'])
thisCommand.addParameter("liketype","Likelihood type",mandatory=commandDefiner.OPTIONAL,defaultValue="unbinned",possiblevalues=['unbinned','binned'])
thisCommand.addParameter("clul","Upper bound confidence level", mandatory=commandDefiner.OPTIONAL,defaultValue= 0.95)
...
thisCommand.addParameter("clobber","Overwrite output file? (possible values: 'yes' or 'no')",mandatory=commandDefiner.OPTIONAL,defaultValue="yes")
thisCommand.addParameter("verbose","Verbose output (possible values: 'yes' or 'no')",mandatory=commandDefiner.OPTIONAL,defaultValue="yes")
thisCommand.addParameter("figure","Matplotlib figure for the interactive mode",mandatory=commandDefiner.OPTIONAL,defaultValue=None,partype=commandDefiner.PYTHONONLY)
thisCommand.addParameter("flemin","Lower bound energy for flux or upper limit computation", mandatory=commandDefiner.OPTIONAL,defaultValue=100,partype=commandDefiner.PYTHONONLY)
thisCommand.addParameter("flemax","Upper bount energy for flux or upper limit computation", mandatory=commandDefiner.OPTIONAL,defaultValue=1000,partype=commandDefiner.PYTHONONLY)
